    ...
    >
      <div className={cn(dashboardIconChip, "mb-6 h-16 w-16 rounded-2xl")}>
        {isSearch ? (
          ...
          <Building2 size={24} strokeWidth={1.8} className="text-[#8B5CF6]" />
        )}
      </div>

      <h2 className="text-xl font-semibold tracking-tight text-gray-950 dark:text-zinc-100">
        {isSearch ? "No matching clients" : "Add your first client"}
      </h2>
      <p className="mt-2 max-w-md text-sm leading-relaxed text-gray-500 dark:text-zinc-400">
        {isSearch
          ? "Nothing matches your search or filters. Try broadening your criteria or reset to see all clients."
          : "Client workspaces hold subscription plans, player limits, and billing. Create one to get started."}
      </p>
      ...
      {!isSearch ? (
        <p className="mt-8 flex items-center gap-1.5 text-xs text-gray-400">
          ...
          Plans, limits, and invoices in one place
        </p>
      ) : null}
    </div>
  );
